...
Projects 原始数据：
  ...
  nowBuilding      = status='Active' → UpdatedAt DESC（活跃度优先）→ 取前 4
                     → 每条查 PROJECT_PROGRESS[title] ?? DEFAULT_PROGRESS
                     （注：title 精确匹配，标题变更会导致 progress 失效，
                       后续建议改为 id 匹配，短期可接受）

Articles 原始数据（status=1）：
  featuredArticle  = view_count DESC → 第 1 篇
  latestArticles   = publish_time DESC → 按 id 排除 featuredArticle → 取前 4
...
```typescript
// key = 项目 title 精确匹配
// ⚠️ 风险：标题变更会导致 progress 失效，后续建议改为 id 匹配
...
| HomeCTA | 无 props | 纯常量驱动，组件内直接 import 常量，无需父组件传参 |
