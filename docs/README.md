# 项目文档中心

当前文档体系已经收缩为 AI 优先的最小集合。

如果是 AI / agent 接手任务，请按这个顺序读取：

1. `../AGENTS.md`
2. `PROJECT_OVERVIEW.md`
3. `PROJECT_STRUCTURE_GUIDE.md`

## 当前保留的核心目录

- `architecture/`
- `api/`
- `config/`
- `deployment/`
- `design-system/`
- `development/`
- `modules/`

## 当前保留的核心手册

- [项目概览](./PROJECT_OVERVIEW.md)
- [项目结构定位指南](./PROJECT_STRUCTURE_GUIDE.md)
- [开发规范](./development/DEVELOPMENT_GUIDELINES.md)
- [样式架构](./development/STYLE_ARCHITECTURE.md)
- [样式管理提醒](./development/STYLE_MANAGEMENT_REMINDER.md)
- [模块系统说明](./architecture/README_MODULES.md)
- [模块开发指南](./development/MODULE_DEVELOPMENT_GUIDE.md)
- [API 配置](./config/API_CONFIG.md)
- [模块系统 API](./api/MODULE_SYSTEM_API.md)
- [部署说明](./deployment/README.md)
- [快速开始](./deployment/QUICK_START.md)

一次性阶段记录不要写入核心手册，完成后直接删除。
