...
- ✅ 公共前台页面默认保留 SSR；只有明确依赖纯客户端能力且确认收益更大时，才允许关闭 SSR，并需要在代码或说明中写明原因
- ✅ 内容读取、Markdown 集合查询、聚合计算等优先放到 `server/api`、Nitro route 或后端接口中处理，页面层只消费轻量 JSON 数据
- ✅ 图表、Markdown 编辑器、3D 场景、富媒体播放器等重型依赖必须优先使用异步组件、动态 `import()` 或进入可视区后再加载，禁止默认进入首屏公共包
- ✅ 新增全局 `head` 资源、全局 CSS、全局插件前，必须先确认是否可以按页面或按组件加载；页面专属样式不得默认挂到全局入口
- ✅ 接口请求避免串行瀑布流；多个独立请求应优先并发、限流或改为服务端聚合，避免首屏和切页阶段逐个等待
- ✅ 图标、字体、图表库等公共资源优先复用项目现有能力；新增第三方大依赖时，必须先评估是否与现有方案重复
- ✅ 涉及首页、公共布局、全局依赖、构建配置的修改后，至少执行一次 `npm run build`，必要时对比 chunk 或构建产物变化，确认没有明显包体回退
- ✅ 新增依赖或引入重量级功能时，需要同时说明加载策略：是否懒加载、是否仅后台使用、是否进入全局包、是否影响首屏
