<template>
  ...
  <div class="min-h-screen">
    <!-- 调试信息 -->
    <div id="app-debug" style="display: none;">App.vue 已渲染, 路径: {{ route.path }}</div>
...
console.log('[App.vue] 执行, 当前路由:', route.path)

// 判断是否应该显示顶部导航栏
// 不显示导航栏的页面：登录页、所有后台管理页面等特殊页面
const shouldShowHeader = computed(() => {
  const path = route.path
  console.log('[App.vue] shouldShowHeader 计算, path:', path)
  // 登录页不显示导航栏
  if (path === '/admin/login') {
    return false
  }
  ...
  if (path === '/admin' || path.startsWith('/admin/')) {
    console.log('[App.vue] 后台页面，隐藏 Header')
    return false
  }
  // 其他所有页面都显示导航栏
  return true
