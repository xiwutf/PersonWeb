...
describe('inline copy edit UI guards', () => {
  ...
  it('editable chrome class only applied for admin session', () => {
    ...
    expect(src).toContain("if (canEditContent.value) classes.push('inline-edit--editable')")
    expect(src).toContain('useAdminSession')
  })

  it('thought category page can add sentences', () => {
    const src = readFileSync(resolve(root, 'pages/life/thoughts/[category].vue'), 'utf8')
    expect(src).toContain('LifeAdminAddButton')
    expect(src).toContain('LifeComposer')
    expect(src).toContain('saveThoughtItems')
  })
