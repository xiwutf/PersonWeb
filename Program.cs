...
namespace EmQuantApiCSharpSample
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                while (str != "EXIT")
                {
                    ...
                    if (str == "STOP") EmStop();
                    if (str == "MA") ManualActivate(userName, passWord);
        ...
        static void ManualActivate(string strName, string strPass)
        {
            ...
            EmQuantAPI.setserverlistdir("lib");
        ...
        //流量查询函数
        //funcname:单个函数名称，不传或为空时表示查询所有 indicators：查询的指标列，多个用英文逗号分隔，不传或为空则查询所有字段
        //options:可传StartDate和EndDate 用完需使用releasedata(pEQCtrData)
        static void EmDataStatistics()
        {
            IntPtr pEqData = IntPtr.Zero;
            ...
            try
            {
                ...
                OptionEnums.EQErr errCode = EmQuantAPI.EmDataStatistics(strFuncName, strIndicators, strOption, out pEqData);
                if (errCode == OptionEnums.EQErr.EQERR_SECUSS)
                {
                    StringBuilder strB = new StringBuilder();
                    object objData = Marshal.PtrToStructure(pEqData, typeof(Structs.EQCtrData));
